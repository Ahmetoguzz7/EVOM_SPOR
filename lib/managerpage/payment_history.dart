...
class _PaymentReminderScreenState extends State<PaymentReminderScreen>
  ...
  // =========================================================================
  // GRUP CACHE — StudentSearchScreen ile AYNI MANTIK (SADECE "TRUE")
  // =========================================================================
  void _loadGroupCache() {
    _studentGroupCache.clear();

    // ========== DEBUG ==========
    print("=== TOPLAM students: ${widget.students.length}");
    print("=== TOPLAM groupStudents: ${widget.groupStudents.length}");
    print("=== TOPLAM groups: ${widget.groups.length}");

    if (widget.students.isNotEmpty) {
      final s = widget.students.first;
      print("=== İLK student: app='${s.app}' role='${s.role}'");
    }
    if (widget.groupStudents.isNotEmpty) {
      final gs = widget.groupStudents.first;
      print(
        "=== İLK groupStudent: student_id='${gs.student_id}' groups_id='${gs.groups_id}' is_active='${gs.is_active}'",
      );
    }
    if (widget.groups.isNotEmpty) {
      final g = widget.groups.first;
      print("=== İLK group: groups_id='${g.groups_id}' name='${g.name}'");
    }

    // Eşleşme kontrolü
    if (widget.students.isNotEmpty && widget.groupStudents.isNotEmpty) {
      final firstStudentApp = widget.students.first.app;
      final matches = widget.groupStudents
          .where((gs) => gs.student_id == firstStudentApp)
          .toList();
      print(
        "=== İlk öğrenci için groupStudent eşleşmesi: ${matches.length} adet",
      );
      if (matches.isNotEmpty) {
        print("=== Eşleşen is_active: '${matches.first.is_active}'");
      }
    }
    // ========== DEBUG SON ==========
    ...
    for (var student in widget.students) {
      ...
      // StudentSearchScreen'deki gibi SADECE "TRUE" kontrolü
      final groupRelations = widget.groupStudents.where((gs) {
        if (gs.student_id != student.app) return false;
        final active = gs.is_active.toString().toUpperCase();
        return active == "TRUE"; // ← StudentSearchScreen ile AYNI
      }).toList();
  ...
  Future<void> _filterStudentsAsync() async {
    ...
    // 🔥 GRUP CACHE'İNİ YENİDEN OLUŞTUR
    _loadGroupCache();
  ...
  Future<void> _sendReminderToAll(
    ...
    // Sadece student rolündeki öğrencileri al
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: isLoading
          ...
          : Column(
              children: [
                // ── Filtre Bölümü (sabit, kaymaz) ──
                Container(
                  color: _surface,
                  padding: const EdgeInsets.fromLTRB(12, 6, 12, 0),
                  child: Column(
                    children: [
                      // Arama + Grup filtresi yan yana
                      Row(
                        children: [
                          // Arama kutusu
                          Expanded(
                        ...
                        tabs: [
                          Tab(
                            child: Row(
                              ...
                              mainAxisSize: MainAxisSize.min,
                              children: [
                                const Icon(Icons.looks_one_outlined, size: 13),
                                const SizedBox(width: 4),
                                const Text("1-14 Gün"),
                                const SizedBox(width: 4),
                                _tabBadge(
                          ...
                          Tab(
                            child: Row(
                              ...
                              mainAxisSize: MainAxisSize.min,
                              children: [
                                const Icon(Icons.looks_two_outlined, size: 13),
                                const SizedBox(width: 4),
...
class _PaymentReminderScreenState extends State<PaymentReminderScreen>
  ...
  void initState() {
    super.initState();
    _tabController = TabController(length: 2, vsync: this);
    _loadAllData(); // 🔥 ARTIK KENDİ VERİLERİNİ ÇEKİYOR
    _searchController.addListener(_onSearchChanged);
  }
  ...
  // =========================================================================
  // 🚀 TÜM VERİLERİ PARALEL OLARAK ÇEK (StudentSearchScreen'deki gibi)
  // =========================================================================
  void _loadAllData() {
    setState(() => isLoading = true);
  ...
  // =========================================================================
  // GRUP CACHE — StudentSearchScreen ile AYNI MANTIK
  // =========================================================================
  void _loadGroupCache() {
    _studentGroupCache.clear();
  ...
  // =========================================================================
  // BİLDİRİM GÖNDERİMİ (Aynı kalacak)
  // =========================================================================
  Future<void> _sendReminder(Users student) async {
    if (student.role.trim().toLowerCase() != "student") {
      _showSnack("❌ Sadece öğrencilere hatırlatma gönderilebilir!", _danger);
