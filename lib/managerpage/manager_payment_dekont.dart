/*
  ...
  class _PaymentScreenState extends State<PaymentScreen> {
    ...
    // Her eleman: { 'date': DateTime, 'fee': double, 'oldFee': double }
    // KAYIT TARİHİ ARTIK BU LİSTEYE EKLENMİYOR
    ...
    // =========================================================================
    // 🔥 AYA ÖZEL ÜCRET HESAPLAMA — DÜZELTİLDİ
    //
    // KURAL:
    //   - Güncelleme yapılan ayın KENDİSİ ve ÖNCESİ → eski ücret (oldFee)
    //   - Güncelleme yapılan aydan SONRA              → yeni ücret (fee)
    //
    // ÖRNEK: Haziran 2025'te 2500→3500 yapıldıysa
    //   Haziran 2025 ve öncesi → 2500 TL
    //   Temmuz 2025 ve sonrası → 3500 TL
    //
    // FİX 1: Loop artık ilk eşleşmede durur, ezmez
    // FİX 2: Kayıt tarihi bu listede yok, karışma olmaz
    // =========================================================================
    double _getStudentMonthlyFeeForMonth(int year, int month) {
      ...
      // Kayıt öncesi aylar → 0
      if (year < registrationYear ||
          (year == registrationYear && month < registrationMonth)) {
        return 0;
      }

      // Güncelleme geçmişi yok → direkt güncel ücret
      if (_feeUpdateHistory.isEmpty) return monthlyFee;

      // Eskiden yeniye sırala (zaten sıralı olmalı ama garanti edelim)
      final updates = List<Map<String, dynamic>>.from(
        _feeUpdateHistory,
      )..sort((a, b) => (a['date'] as DateTime).compareTo(b['date'] as DateTime));

      // Kronolojik sıraya göre tara:
      // İlk "hedef ay bu update'in ayından SONRA DEĞİL" noktasında → oldFee dön
      for (var update in updates) {
        final uDate = update['date'] as DateTime;
        final uYear = uDate.year;
        final uMonth = uDate.month;

        final bool targetIsAfterUpdateMonth =
            (year > uYear) || (year == uYear && month > uMonth);

        if (!targetIsAfterUpdateMonth) {
          // Hedef ay, bu update'in ayı veya öncesi → güncelleme öncesi ücret
          return update['oldFee'] as double;
        }
        // Hedef ay bu update'ten sonra → bir sonraki update'e bak
      }

      // Tüm update'lerden sonraki bir aydaysak → en güncel ücret
      return monthlyFee;
    }
    ...
    List<DateTime> _getMonthsOfYear(int year) {
      ...
      for (int m = 1; m <= 12; m++) {
        ...
        if (year == registrationYear) {
          if (m < registrationMonth) continue;
        }
    ...
    // =========================================================================
    // 🔥 PARALEL VERİ YÜKLEME — DÜZELTİLDİ
    //
    // FİX 1: payments tarihe göre sıralanarak işleniyor (initialFee doğru hesaplanıyor)
    // FİX 2: Kayıt tarihi _feeUpdateHistory'ye EKLENMIYOR (karışma önlendi)
    // FİX 3: Her payment'ın oldFee'si kendi note'undan güvenle alınıyor
    // =========================================================================
    Future<Map<String, dynamic>> _loadAllDataParallel() async {
      ...
      try {
        ...
        // FİX 1: Önce tarihe göre eskiden yeniye sırala
        ...
        // FİX 2: Kayıt tarihini listeye EKLEME — önceden buraya eklenen
        // kayıt tarihi girdisi _getStudentMonthlyFeeForMonth'ta yanlış
        // sonuçlara yol açıyordu. Kayıt tarihi sadece UI'da gösterim için
        // widget.student.created_at'ten okunuyor, hesaplama etkilenmiyor.

        // Sıralı olduğundan emin ol (zaten sıralı ama garanti)
        _feeUpdateHistory.sort(
          (a, b) => (a['date'] as DateTime).compareTo(b['date'] as DateTime),
        );

        print("📋 Fee update geçmişi: ${_feeUpdateHistory.length} kayıt");
        for (var u in _feeUpdateHistory) {
          print(
            "  → ${(u['date'] as DateTime).year}-${(u['date'] as DateTime).month}: "
            "${u['oldFee']} TL → ${u['fee']} TL",
          );
        }
                      ...
                      : () async {
                          ...
                          if (newFee > 0 && newFee != monthlyFee) {
                            ...
                            if (ok) {
                              ...
                              // Note formatı: "FEE_UPDATE: 3500 (Eski: 2500 TL)"
                              // Bu format _loadAllDataParallel'deki regex ile uyumlu
  ...
  double _getStudentMonthlyFeeForMonth(int year, int month) {
    ...
    for (var update in updates) {
      ...
      if (!targetIsOnOrAfterUpdateMonth) {
        // Hedef ay bu update'in ayından önce → güncelleme öncesi ücret
        return update['oldFee'] as double;
      }
      // Hedef ay bu update'ten sonra → bir sonraki update'e bak
    }
