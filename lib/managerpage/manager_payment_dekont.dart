/*import 'dart:async';
import 'package:EVOM_SPOR/managerpage/manager_offline/offline_payment_service.dart';
...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  late final OfflinePaymentService _offlinePaymentService;
  ...
  Future<void> _initOfflineService() async {
    ...
    await _offlinePaymentService.init();
    if (_isMounted) {
      setState(() {
  ...
  // =========================================================================
  // 🔥 LOKAL VERİ YÜKLEME (OFFLINE-FIRST)
  // =========================================================================
  Future<Map<String, dynamic>> _loadAllDataParallel() async {
    ...
    try {
      ...
      final localPayments = _repo.allPayments;
      ...
      final allFeeUpdatePayments = localPayments
          .where(
            (p) =>
      ...
      final overridePayments = localPayments.where((p) {
                ...
                onPressed: isSubmitting
                    ...
                    : () async {
                        ...
                        if (newFee > 0 && newFee != monthlyFee) {
                          ...
                          final oldMonthlyFee = monthlyFee;

                          final feeUpdatePayment = Payment(
                            payments_id:
                                "local_fee_${DateTime.now().millisecondsSinceEpoch}",
                            ...
                            note:
                                "FEE_UPDATE: ${newFee.toStringAsFixed(0)} (Eski: ${oldMonthlyFee.toStringAsFixed(0)} TL)",
                          );

                          _repo.allPayments.add(feeUpdatePayment);
                          paymentHistory.insert(0, feeUpdatePayment);

                          _feeUpdateHistory.add({
                            'date': DateTime.now(),
                            'fee': newFee,
                            'oldFee': oldMonthlyFee,
                          });
                          _feeUpdateHistory.sort(
                          ...
                          // 🔥 DÜZELTİLMİŞ KISIM - final değişken sorunu çözüldü
                          final userIndex = _repo.allUsers.indexWhere(
                            (u) => u.app == widget.student.app,
                          );
                          if (userIndex != -1) {
                            // Yeni bir User nesnesi oluştur (immutable taklidi)
                            final oldUser = _repo.allUsers[userIndex];
                            final updatedUser = Users(
                              ...
                              phone: oldUser.phone,
                              role: oldUser.role,
                              b_date: oldUser.b_date,
                              mother_name: oldUser.mother_name,
                              father_name: oldUser.father_name,
                              profile_photo_url: oldUser.profile_photo_url,
                              created_at: oldUser.created_at,
                              amount: newFee.toStringAsFixed(0),
                              branches_id: '',
                              password_hash: '',
                              last_login: '',
                              is_active: '', // Yeni ücret
                            );
                            _repo.allUsers[userIndex] = updatedUser;
                          }
                          ...
                          unawaited(
                            _offlinePaymentService
                                .savePayment(feeUpdatePayment)
                                .then((success) async {
                                  if (success && _isMounted) {
                                    try {
                                      await GoogleSheetService.updateUserAmount(
                                        widget.student.app,
                                        newFee,
                                      );
                  ...
                  Container(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          const Text(
                            "Değiştirmek İstediğiniz Ayı Seçin",
                ...
                ElevatedButton(
                  onPressed: isSubmitting
                      ...
                      : () async {
                          ...
                          final List<Payment> newOverridePayments = [];
                          ...
                          for (int month in selectedMonths) {
                            ...
                            final overridePayment = Payment(
                              ...
                              payment_method: "Sistem",
                              note:
                                  "OVERRIDE: $generatedCode (Toplu güncelleme)",
                            );

                            _repo.allPayments.add(overridePayment);
                            paymentHistory.insert(0, overridePayment);
                            newOverridePayments.add(overridePayment);
                            _feeOverrides[yearMonth] = newFee;
                          }
                          ...
                          // Arka planda offline kaydet
                          unawaited(
                            _offlinePaymentService
                                .savePaymentsBatch(newOverridePayments)
                                .then((success) async {
                                  if (success && _isMounted) {
                                    try {
                                      GoogleSheetService.invalidateCache(
  ...
  Future<void> _processPaymentForMissingMonth(
    ...
    final newPayment = Payment(
      payments_id: "local_${DateTime.now().millisecondsSinceEpoch}",
      student_id: widget.student.app,
      groups_id: selectedGroupId!,
      recorded_by: "Admin",
    ...
    _repo.allPayments.add(newPayment);
    ...
    _showSnackBar("✅ $monthName ödemesi kaydedildi! (Senkronize ediliyor...)");

    unawaited(
      _offlinePaymentService.savePayment(newPayment).then((success) {
        if (success && _isMounted) {
...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  bool _historicalDebtExpanded = false;
  final Set<String> _deletingIds = {}; // çift silmeyi önler
              ...
              ElevatedButton(
                onPressed: isSaving
                    ...
                    : () async {
                        ...
                        final newMethod = selectedMethodEdit;
                        final newDate = _formatDateForDB(selectedDateEdit);

                        if (newAmount.isEmpty ||
                            double.tryParse(newAmount) == null) {
                          _showSnackBar(
                            "❌ Geçerli bir tutar girin!",
                            isError: true,
                          );
                        ...
                        // 1. Optimistic: UI'ı anında güncelle
                        final idx = paymentHistory.indexWhere(
                          (p) => p.payments_id == payment.payments_id,
                        );
                        final repoIdx = _repo.allPayments.indexWhere(
                          (p) => p.payments_id == payment.payments_id,
                        );
                        ...
                        final updatedPayment = Payment(
                          ...
                          groups_id: selectedGroupId ?? payment.groups_id,
                        ...
                        if (_isMounted) {
                          setState(() {
                            if (idx >= 0) paymentHistory[idx] = updatedPayment;
                            if (repoIdx >= 0)
                              _repo.allPayments[repoIdx] = updatedPayment;
                          });
                          if (mounted) Navigator.pop(context);
                          _showSnackBar("✅ Ödeme güncellendi!");
                        }
                        ...
                        try {
                          ...
                          final success =
                              await GoogleSheetService.updatePayment(
                                payment.payments_id,
                                updateData,
                              );
                          ...
                          if (!success && _isMounted) {
                            // Geri al
                            setState(() {
                              if (idx >= 0) paymentHistory[idx] = payment;
                              if (repoIdx >= 0)
                                _repo.allPayments[repoIdx] = payment;
                            });
                            _showSnackBar(
                              "❌ Güncelleme başarısız, eski kayıt geri yüklendi!",
                              isError: true,
                            );
                          } else if (success) {
                            GoogleSheetService.invalidateCache('payments');
  ...
  void _confirmDeletePayment(Payment payment) async {
    if (!_isMounted) return;
    // Aynı ödeme zaten siliniyor mu?
    if (_deletingIds.contains(payment.payments_id)) return;
    ...
    if (confirm != true || !_isMounted) return;

    // Guard: tekrar basılmasın
    _deletingIds.add(payment.payments_id);

    // 1. Önce UI'dan anında kaldır (optimistic)
    if (_isMounted) {
      setState(() {
        paymentHistory.removeWhere((p) => p.payments_id == payment.payments_id);
        _repo.allPayments.removeWhere(
          (p) => p.payments_id == payment.payments_id,
        );
      });
      // Dialog açıksa kapat
    ...
    // 2. Arka planda DB'ye yaz
    try {
      final success = await GoogleSheetService.deletePayment(
        payment.payments_id,
      );
      if (!success && _isMounted) {
        // DB başarısız olduysa geri ekle
        setState(() {
          _repo.allPayments.add(payment);
          paymentHistory.insert(0, payment);
          paymentHistory.sort((a, b) => b.paid_date.compareTo(a.paid_date));
        });
        _showSnackBar("❌ Silme başarısız, kayıt geri yüklendi!", isError: true);
      } else if (success) {
        // Sadece local cache'i güncelle, tam refresh yapmaya gerek yok
        GoogleSheetService.invalidateCache('payments');
      }
    } catch (e) {
      if (_isMounted) {
        ...
        _showSnackBar("❌ Hata oluştu, kayıt geri yüklendi!", isError: true);
      }
    } finally {
      _deletingIds.remove(payment.payments_id);
    }
  }
