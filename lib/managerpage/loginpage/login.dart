/*
// lib/signup/advanced_signup.dart
...
class _AdvancedSignUpPageState extends State<AdvancedSignUpPage> {
  ...
  // 🔥 Güncelleme için seçilen öğrenci
  ...
  final TextEditingController _CreadetDateController = TextEditingController();
  ...
  File? _profileImage;
  String _existingPhotoUrl = ""; // Mevcut fotoğraf URL'si (güncelleme için)
  ...
  // 🔥 SEÇİLEN ÖĞRENCİYİ FORMA YÜKLE
  void _loadStudentToForm(Users student) {
    setState(() {
      _selectedStudent = student;
      _nameController.text = student.first_name;
      _surnameController.text = student.last_name;
      _emailController.text = student.email;
      _phoneController.text = student.phone;
      _passwordController.text = "";
      _birthDateController.text = student.b_date.split('T')[0];
      _CreadetDateController.text = student.created_at.split('T')[0];
      _selectedBranchId = student.branches_id;
      _selectedRole = student.role;
      _selectedGroupId = _groups.isNotEmpty ? _groups.first.groups_id : "";
      _healthProblemsController.text = "";

      // 🔥 MEVCUT FOTOĞRAF
      _existingPhotoUrl = student.profile_photo_url;
      _profileImage = null;
    ...
    _showSnackBar("${student.first_name} ${student.last_name} yükleniyor...");
  }
  ...
  Future<void> _updateStudent() async {
    ...
    if (_profileImage != null) {
      try {
        final fileName =
            "profile_${_selectedStudent!.app}_${DateTime.now().millisecondsSinceEpoch}.jpg";
        photoUrl = await GoogleSheetService.uploadImageToDrive(
          _profileImage!,
          fileName,
          "profile_photos",
          targetUserId: _selectedStudent!.app,
    ...
    Map<String, dynamic> updatedData = {
      ...
      "creadet_at": _CreadetDateController.text.trim(),
      "amount": "",
    ...
    if (success) {
      _showSnackBar("✅ Öğrenci bilgileri güncellendi!");
      await _loadData();
      _clearForm();
      setState(() {
  ...
  Future<void> _selectCreatDate() async {
    final DateTime? picked = await showDatePicker(
      context: context,
      initialDate: DateTime.now().subtract(const Duration(days: 365 * 7)),
      firstDate: DateTime(1950),
  ...
  Future<void> _handleRegister() async {
    ...
    Map<String, dynamic> userInfo = {
      ...
      "created_at": _CreadetDateController.text.trim().isEmpty
          ? DateTime.now().toIso8601String().substring(0, 10)
          : _CreadetDateController.text.trim(),
          ...
          : SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  // 🔥 GÜNCELLEME MODUNDA ÖĞRENCİ ARAMA
                  if (_mode == "update") ...[
                    _buildSearchSection(),
                    const SizedBox(height: 20),
                  ],
                  ...
                  Form(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 20),
                        if (_selectedRole == 'student') _buildHealthSection(),
                        const SizedBox(height: 20),
                        _buildParentSection(),
                        const SizedBox(height: 20),
                        _buildSubmitButton(),
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                          leading: CircleAvatar(
                            ...
                            child: Text(student.first_name[0].toUpperCase()),
                          ),
                          title: Text(
                          ...
                          onTap: () => _loadStudentToForm(student),
                        );
                      },
                    ),
                  ),
  ...
  Widget _buildCreatDateField() {
    return InkWell(
      onTap: _selectCreatDate,
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 16),
        decoration: BoxDecoration(
          border: Border.all(color: Colors.grey.shade300),
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Text(
                _CreadetDateController.text.isEmpty
                    ? "Hesap oluşturma Tarihini Seçin"
                    : _CreadetDateController.text,
                style: TextStyle(
                  color: _CreadetDateController.text.isEmpty
                      ? Colors.grey.shade500
                      : Colors.black,
                ),
              ),
  ...
  Widget _buildHealthSection() {
    if (_selectedRole != 'student') return const SizedBox();
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        onPressed: _isLoading
            ? null
            : (_mode == "register" ? _handleRegister : _updateStudent),
        child: _isLoading
            ? const SizedBox(
                width: 22,
                height: 22,
...
class _AdvancedSignUpPageState extends State<AdvancedSignUpPage> {
  ...
  // 🔥 ÖĞRENCİ ARA - DÜZELTİLDİ (Tüm roller)
  ...
  // 🔥 SEÇİLEN KULLANICIYI FORMA YÜKLE
  ...
  Future<void> _selectCreatedDate() async {
    ...
    if (picked != null) {
      ...
      String day = picked.day.toString().padLeft(2, '0');
      setState(() {
        _createdDateController.text = "$year-$month-$day";
      });
    }
  }
          ...
          : SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  // GÜNCELLEME MODUNDA KULLANICI ARAMA
                  if (_mode == "update") ...[
                    _buildSearchSection(),
                    const SizedBox(height: 20),
                  ],
