...
class _TakeAttendanceScreenState extends State<TakeAttendanceScreen> {
  ...
  void initState() {
    super.initState();
    _dataFuture = _loadAllDataParallel(); // 🔥 PARALEL VERSİYON
  }
  ...
  Future<Map<String, dynamic>> _loadAllDataParallel() async {
    ...
    try {
      ...
      stopwatch.stop();
      print(
        "⏱️ TakeAttendanceScreen verileri PARALEL olarak ${stopwatch.elapsedMilliseconds}ms'de yüklendi",
      );
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: const Text(
            "Yoklama Alma ve Düzenleme Paneli",
...
class _TakeAttendanceScreenState extends State<TakeAttendanceScreen> {
  ...
  void initState() {
    super.initState();
    _dataFuture = _loadAllDataParallel();
  }
  ...
  String _formatDateLongTurkish(DateTime date) {
    final formatter = DateFormat('dd MMMM yyyy', 'tr_TR');
    return formatter.format(date);
  }

  // Ay adını sayıdan al (1 = Ocak, 2 = Şubat...)
  ...
  // 🔥 DÜZENLENEN STATUS BAR (Türkçe tarih)
  Widget _buildStatusBar() {
