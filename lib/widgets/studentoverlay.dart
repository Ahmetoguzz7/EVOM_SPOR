// lib/widgets/studentoverlay.dart
...
class StudentLoadingScreen extends StatefulWidget {
  final Users user;
  const StudentLoadingScreen({super.key, required this.user});
...
class _StudentLoadingScreenState extends State<StudentLoadingScreen>
  ...
  final List<MapEntry<String, String>> _loadingSteps = [
    ...
    const MapEntry("👤 Profil Bilgileri", "Profiliniz yükleniyor..."),
    const MapEntry("👥 Grup Bilgileri", "Antrenman grubunuz hazırlanıyor..."),
    const MapEntry(
      "🏃 Antrenman Programı",
      "Haftalık programınız yükleniyor...",
    ),
    const MapEntry("🖼️ Görseller", "Profil fotoğrafları ön yükleniyor..."),
    const MapEntry("✨ Hazır!", "Ana sayfaya yönlendiriliyorsunuz..."),
  ];

  final List<String> _tips = [
    "💡 İpucu: Antrenmanlarına düzenli katılım göster!",
    "💡 İpucu: Hedeflerini belirle ve onlara odaklan!",
    "💡 İpucu: Beslenmene dikkat et, başarının %70'i beslenmedir!",
    "💡 İpucu: Yeterli uyku, performansını artırır!",
    "💡 İpucu: Antrenörünle iletişimde kal, gelişimini takip et!",
  ];
    ...
    try {
      ...
      if (mounted) {
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (_) => UserInterface(user: widget.user)),
        );
      }
    } catch (e) {
